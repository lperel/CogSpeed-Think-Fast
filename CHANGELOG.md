## V699rev178 — calibration pending-correct-tap freeze fix
...
- Rev176 unified self-paced calibration phase (replaces per-mode calibration logic):
  ...
  - Verification: app.js passes node --check after each of the five implementation groups. Live grep confirms zero remaining readers of the six retired DEFAULTS keys; the only remaining textual references to the retired keys are in the V699rev178 retirement-comment blocks. All six new DEFAULTS keys are read in the new flow with `Math.max(1, Number(settings.X) || DEFAULTS.X)` so admin-blank or admin-zero inputs fall through to a safe minimum rather than producing NaN-driven flow corruption.
  - Version stamps bumped V699rev175 → V699rev178 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine / manifest.json name. APP_VERSION remains V699. No CPA, audio, scheduler-notice, copyright/license, or technical-overview content changed.
