## V699rev183 — last tutorial step: hide redundant SKIP TO TEST button
- The 7-step tutorial's button row carried three buttons on every step: Back / Next / SKIP TO TEST. On the last step (Profile Scheduler) the middle button reads "▶ Start Test!" in green and starts the test, so the third "SKIP TO TEST" button became functionally redundant — both buttons routed to the same place. Reported by Layne Apr 28 2026: "last tutorial page has duplicate start buttons. I don't see a reason for this."
- Fix: in `tutSetStep()` (the function that updates button labels and styles per step), added a small block at the end that on the last step (`n === TUT_STEPS.length-1`) sets `tutSkipBtn.style.display = "none"` and collapses `#tutorialButtonsRow` from `grid-template-columns:1fr 1fr 1fr` to `1fr 1fr`. On every other step, the block restores the 3-column layout and `display:""`. The skip button's click handler stays attached (a hidden button can't be tapped, so this is harmless), so reverting the change is a one-line edit if ever desired.
- Steps 0 through 5 are unchanged: still three buttons (Back / Next / SKIP TO TEST). Only step 6 (the last step, Profile Scheduler) shows two buttons (Back / ▶ Start Test!) at half width each.
- No JavaScript behavior change beyond the cosmetic button-row update. No CSS rules added or removed. No admin defaults touched.
- Version stamps bumped V699rev182 → V699rev183 in app.js APP_REV_STAMP, sw.js RELEASE, manifest.json name, index.html title/badge/statusLine/cache-bust query.

## V699rev182 — Pattern Refresher mobile portrait gear-image visibility fix
- The Pattern Refresher's `.ref-grid` rule has used `grid-template-columns:1fr 1fr 1fr` (3 columns) since well before the gear-PNG implementation was introduced. At a typical mobile-portrait viewport (iPhone 13/14/15 Pro = 390px wide), three columns give each card only ~109px of width, which collapses the inner gear box to ~33px and the gear `<img>` to ~42px. At that size the rusty-metal gear texture is invisible behind the dot/line marks — users see only the white shapes floating on the dark blue card. Landscape (844px), tablet (768px+), and desktop layouts all give the cards enough width that the gears render correctly, which is why the bug only manifests in mobile portrait. This is a pre-existing layout issue, not a regression.
- Fix: one new `@media(orientation:portrait) and (max-width:767px)` rule placed immediately after the existing `@media(max-width:360px)` block in the inline `<style>` of `index.html`. The rule overrides `.ref-grid` to use `grid-template-columns:1fr 1fr` in mobile portrait only. Six cards lay out as 2×3 in portrait (which scans naturally on a tall narrow phone) and stay 3×2 in landscape. The supersession of the very-narrow-Android `@media(max-width:360px)` rule (which forces 3 columns) is intentional in portrait — that legacy rule predates the gear-PNG implementation and was tuned for the old SVG-only layout where smaller per-card width was acceptable.
- Measured impact (playwright headless): iPhone Pro portrait gear `<img>` size grew from 42×53 px to 79×99 px (+88%). Small Android portrait grew from 29×36 px to 58×73 px (+100%). Landscape (844×390), tablet (768×1024), and desktop (1542×1884) renders are byte-identical before and after the fix (md5sum confirmed).
- Verified across all three Challenge Sets (Standard / Memory / Survival) at iPhone Pro portrait: rusty gears, animal/object icons, and military icons all now render at a clearly readable size.
- No JavaScript changes. No new admin defaults. No retired settings. Single CSS rule addition. Version stamps bumped V699rev181 → V699rev182 in app.js APP_REV_STAMP, sw.js RELEASE, manifest.json name, index.html title/badge/statusLine/cache-bust query.
...
- Rev176 unified self-paced calibration phase (replaces per-mode calibration logic):
  - Specification (Layne, V699 rev 176): the calibration phase is now mode-agnostic — a single self-paced training phase that gives the subject a chance to demonstrate they have learned the test, while also providing refresher practice for experienced users and minimizing spoofing. Pass criteria are uniform across Modes 1, 2, 3, 4: 6 consecutive correct responses AND rolling-mean trial RT < 2600 ms, achieved within 21 measured attempts (post-warmup). Per-trial 10-second no-response cap. 6-second green-hint flash on the correct response cell to assist learning when the subject freezes. Overall 60-second budget on the entire calibration phase. After each WRONG measured response, the rolling mean window resets to empty and the correct cell flashes green; the subject must tap the correct cell to advance (and that wrong response consumes one of the 21 attempt slots). After each CORRECT response, the next new trial is presented immediately.
  - On PASS, the test transitions automatically to the appropriate next phase using the rolling-mean RT as the source baseline:
    • Mode 1 → adaptive machine-paced phase, baseline = rollingMeanRT × initialPacedPercent (admin-configurable, default 1.2)
    • Mode 2 → adaptive machine-paced phase (then sustained + final SP as today), baseline = rollingMeanRT × initialPacedPercent (default 1.2)
    • Mode 3 → new mode3_continued phase: pure self-paced trials with NO green hint and NO per-trial 10s cap; ends via existing mode3MaxDurationMs (default 90000) and mode3TrialLimit (default 150) guards. Scored with current Mode 3 methods.
    • Mode 4 → fixed-baseline machine-paced phase, baseline = rollingMeanRT × mode4BaselineFactor (default 1.1)
  - On FAIL (any of: 60s phase budget exceeded, 21 measured attempts used without pass, 10s per-trial no-response, max-attempts-then-wrong without further attempts available), the speedometer/outcome page renders the headline "FAILED — NEEDS MORE PRACTICE!" (instead of the generic "Failed") and the disposition box shows an S-PFS-keyed safety-critical recommendation drawn from the SUBJECT'S SELF-REPORTED pre-test S-PFS (state.samnPerelli at start), not from any test-derived score (since calibration failure means no valid test-derived disposition is computable). Three text bands per Layne's verbatim spec:
    • S-PFS 7 / 6 / 5: "Safe to drive or perform any safety-critical task."
    • S-PFS 4:        "Functioning slightly less than normal. Caution. Mild fatigue detected."
    • S-PFS 3:        "Functioning starting to slow. Strong recommendation: rest before continuing. Do not drive or perform any safety-critical task without a break."
    • S-PFS 2 / 1:    "Unable to function / Definitely unsafe. Stop! You are not safe to drive or perform any safety-critical task. Rest is required."
    The speedometer "Back to Start" button is dynamically relabeled to "Continue" for failed-calibration sessions per Layne's spec ("CAN GO TO 'CONTINUE' TO RESTART"). The handler is unchanged — it still routes back to the start page where the subject re-presses Start — but the verb is more inviting after a training-only failure.
  - Implementation:
    • Six new admin defaults (DEFAULTS block + ADMIN_FIELDS array, fields 4-9 in the renumbered admin panel): calibrationPassConsecutiveCorrect (6), calibrationPassRtMeanMs (2600), calibrationMaxAttempts (21), calibrationMaxPhaseDurationMs (60000), calibrationHintAfterMs (6000), calibrationPerTrialMaxMs (10000). The legacy admin field numbering for Modes 1-4 (fields 16+) shifted forward by two slots to absorb the new keys; downstream Memory Challenge / Survival Challenge / Mode 2 normative CPA / Personal Baseline field numbers (51+) are unchanged.
    • Six legacy calibration defaults retired (no live readers remain after the rewrite): initialMeasuredCalibrationTrials, calibrationFirstNoResponseMs, calibrationNoResponseMs, calibrationStopErrors, calibrationStopSlowMs, mode4CalibrationTrials. The warm-up trial count (initialUnusedCalibrationTrials, default 1) was kept because the new flow still uses it.
    • New state fields (state object): calibrationConsecCorrect, calibrationRollingCorrectRTs (RT window, rebuilt empty on each wrong measured response — drives the rolling-mean check), calibrationAttemptsUsed (post-warmup attempt budget, both correct and wrong each consume one slot), calibrationPhaseStartMs, calibrationPhaseTimer, calibrationHintTimer, calibrationHintActive, calibrationPendingCorrectTap (true while waiting for the subject to tap the correct cell after a wrong measured response), calibrationFailedFlag (set by failCalibration; carried onto the saved result row so the speedometer can reliably distinguish fail-cal sessions from generic failures without endReason string parsing). All cleared on resetTrialStateOnly.
    • New helpers (placed below failCalibration, anchoring the calibration logic block): armCalibrationPhaseTimer / clearCalibrationPhaseTimer (idempotent 60s phase budget), armCalibrationHintTimer / clearCalibrationHintTimer (per-trial 6s green-hint timer + class sweep), applyCalibrationHintToCorrectCell (force-flashes the correct cell immediately on a wrong measured response, also called by the hint-timer expiry closure), getCalibrationFailDispositionBySpfs (S-PFS → recommendation string lookup; mirrors the structure of getDispositionRecommendation but uses Layne's rev176 wording), getEffectiveSpfsForFailedCalibration (defensive S-PFS lookup with state-fallback for live calls before the result row exists), isCalibrationFailedResult (canonical detector — checks calibrationFailedFlag first, with endReason-prefix fallback for legacy/repaired rows). failCalibration itself now clears phase/hint timers, sets calibrationFailedFlag=true, then routes to finish() — every fail path goes through this single function.
    • handleTap calibration branch fully rewritten with a clean three-mode trial flow (warm-up / measured / pending-correct-tap-after-wrong); the per-mode branching used through V699rev175 (separate Mode 1/2 vs Mode 3 vs Mode 4 paths) was deleted in favor of the unified rules. The pending-correct-tap state ignores wrong cell taps (no flash, no advance) so the subject is gently forced to identify the right answer before continuing.
    • finishCalibration rewritten: PASS-only entry, sources the baseline RT from calibrationRollingCorrectRTs (with calibrationRTs / selfPacedRTs as fallbacks for legacy persisted shape), branches by mode. The slow-calibration-halt branch (avg > calibrationStopSlowMs) was removed because slow performance now fails earlier via the per-trial 10s cap or the 60s phase budget.
    • openTrial calibration block updated for the new attempt-count semantics (total = warmups + max attempts; default 1 + 21 = 22 across all modes) and arms the new phase + hint timers in the rAF callback alongside armNoResponseTimer. New mode3_continued kind added with no hint, no per-trial cap, and a phase label that reflects Mode 3 SP progress.
    • New mode3_continued phase and tap handler. Trial-log rows continue to use phase tag "calibration" so downstream CSV / aggregate / rank-averages code keeps identifying them as Mode 3 self-paced trials per the existing schema (the mode3_continued state.phase value is internal to the trial flow only). Phase ends when calibrationTrialIndex reaches mode3TrialLimit + initialUnusedCalibrationTrials.
    • armNoResponseTimer calibration branch simplified to a flat per-trial cap (calibrationPerTrialMaxMs, default 10000), eliminating the first-trial-vs-later distinction. Calibration no-response timeout now routes through failCalibration() so the run is properly flagged.
    • handleTap allowed-phases gate extended with "mode3_continued" so taps during Mode 3 post-calibration are dispatched to the new handler.
    • CSS keyframe calibrationHintCorrectPulse added in index.html — 1.0s green pulse on the correct response cell. Mirrors the tutorial's tutPairFlashCorrect pattern but tuned for live test usage. The pulse runs on the .resp-btn container (border + filter + box-shadow) so the existing correct-flash / wrong-flash classes (which target svg) can still apply on top without conflict.
    • renderMode2SpeedometerBoxes extended to accept a per-box valueStyle string so the longer calibration-fail recommendation strings (up to ~110 characters) render at 14px with line-height:1.35 instead of clipping at the legacy 20px font.
    • renderSpeedometerOutcome calibration-fail branch overrides the normal Mode-2-only metric box for failed-calibration sessions in ALL FOUR MODES. The override is semantically clean — for failed calibrations no Mode 2 sustained phase ever ran, so no Mode-2 metric values were computable anyway.
    • syncOutcomeStatusText branch swaps the headline to "FAILED — NEEDS MORE PRACTICE!" only for calibration-failed sessions; generic non-cal-fail failures still read "Failed".
  - Persistence: calibrationFailedFlag is stored on the saved result row (boolean) so post-rev176 sessions reliably distinguish failed-calibration runs without endReason heuristics. Pre-rev176 saved rows on a device's localStorage have calibrationFailedFlag === undefined; isCalibrationFailedResult falls back to endReason-prefix matching so legacy rows whose endReason still reads "Calibration performance indicates more practice is needed" or "No response detected — please retest" or any of the rev175 calibration-fail strings render the same way. The new endReason prefixes ("Calibration: …", "Calibration phase exceeded …", "Calibration failed …") are also matched.
  - Surfaces NOT changed (deliberately):
    • The disposition logic for SUCCESSFUL tests (computeDisposition + getDispositionRecommendation) is unchanged. The new getCalibrationFailDispositionBySpfs is a separate function with separate wording per Layne's spec (the recommendations differ — the rev175 success-path strings are written for completed tests with a CPI, while the rev176 fail-path strings are written for tests that did not produce a score and therefore lean on the subject's own pre-test S-PFS rating). Both functions exist side by side; the renderer picks based on isCalibrationFailedResult.
    • The Trial Detail Log CSV phase column continues to read "calibration" for all calibration-phase trials including the new mode3_continued post-pass trials, preserving column-index compatibility for any external scripts joining on the trial log.
    • selfPacedResponseMeanMs (correct-only post-warmup mean introduced in V699rev174) continues to compute the same way; the new flow's wrong-measured-response handling ensures wrong RTs still never enter that array.
    • The CPI / CPA scoring chain, the Personal Baseline qualification rules, and the Mode 2 sustained / final SP phases are entirely unchanged. The rev176 work is confined to the calibration phase and the failed-cal speedometer rendering.
  - Verification: app.js passes node --check after each of the five implementation groups. Live grep confirms zero remaining readers of the six retired DEFAULTS keys; the only remaining textual references to the retired keys are in the V699rev180 retirement-comment blocks. All six new DEFAULTS keys are read in the new flow with `Math.max(1, Number(settings.X) || DEFAULTS.X)` so admin-blank or admin-zero inputs fall through to a safe minimum rather than producing NaN-driven flow corruption.
  - Version stamps bumped V699rev175 → V699rev180 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine / manifest.json name. APP_VERSION remains V699. No CPA, audio, scheduler-notice, copyright/license, or technical-overview content changed.


  - Specification (Layne, V699 rev 175): the existing dispositionLabel ("Functioning normally", "Difficult to function / becoming unsafe", etc.) describes COGNITIVE STATE — useful for research and analysis but not directly actionable for a user. Layne wants a parallel field that gives universal action guidance suitable for ANY safety-critical context (driving, piloting, operating machinery, performing safety-critical work) so a single deployment configuration can serve roadside (Buc-ee's), aviation pre-flight, ER pre-shift, oil-rig start-of-shift, etc. without separate copy. Layne chose option 3 ("Keep both — a 'state' field for the cognitive description AND a new 'recommendation' field for the action guidance"), so the existing dispositionLabel is unchanged and a new dispositionRecommendation field is added.
  - Wording locked in (verbatim per Layne, NOT to be paraphrased without an explicit spec change):
    • disp 7-6-5 (groups together):  "Fit for duty. Safe to drive or perform any safety-critical task."
    • disp 4:  "Caution. Mild fatigue detected. Consider rest before driving or any safety-critical task."
    • disp 3:  "Strong recommendation: rest before continuing. Do not drive or perform any safety-critical task without a break."
    • disp 2-1 (groups together):  "Stop. You are not safe to drive or perform any safety-critical task. Rest is required."
    The 7-6-5 grouping reflects "all three are safe to proceed"; the 2-1 grouping reflects "both are unsafe; the granularity is academic at this point." The 4 and 3 bands are deliberately distinct: 4 is informative ("consider rest"), 3 is forceful ("do not...without a break"). The asymmetry mirrors the underlying safety asymmetry: at 4, performance is mildly degraded but a short break may be enough; at 3, performance is degraded enough that a meaningful break is required, not optional.
  - Implementation:
    • New helper getDispositionRecommendation(spfs) (added immediately after computeDisposition) returns the recommendation string for a given dispositionSpfs (1-7). Defensive: returns null for non-finite, out-of-range, or invalid inputs. Numeric coercion via Number() so "7" works the same as 7. Uses Math.round for non-integer values (3.4 -> "3" recommendation, 3.6 -> "4" recommendation). Pulled into its own function so future deployments (Buc-ee's variant, aviation variant, research-only variant that suppresses recommendations) can override or replace this single function without touching computeDisposition's scoring logic.
    • computeDisposition return shape extended from {code, label, spfs} to {code, label, spfs, recommendation}. The blank object returned for invalid inputs also includes dispositionRecommendation:null so callers can rely on the field always existing on the returned object.
    • POLARITY NOTE added to the computeDisposition comment block. CogSpeed uses a re-oriented Samn-Perelli convention (7=best, 1=worst), opposite the canonical published scale (1=best, 7=worst). Documenting this prevents a future audit or contributor from re-introducing a polarity bug. Same numbers, opposite meanings is exactly the kind of safety-critical bug that hides for years.
  - Persistence migration. dispositionRecommendation is added to:
    • The Mode 2 startup repair block (line 890) — re-runs computeDisposition when the new field is missing on saved Mode 2 sessions, then copies it onto the row.
    • The general legacy-disposition repair block (line 911) — re-runs computeDisposition when ANY of the four disposition fields is missing OR a legacy GREEN/YELLOW/ORANGE/RED color code is detected. Returning users from rev <=174 get the recommendation back-filled on their saved sessions on first rev175 launch. Idempotent on subsequent launches.
    • The result-summary repair check (line 7174) — re-runs when the new field is missing, so the summary always renders with a recommendation even for stale rows that hit the summary path before the startup repair completes.
    • The Mode 2 fallback summary repair (line 7361) — same pattern.
    • The speedometer Mode 2 disposition repair (line 10897) — same pattern.
  - User-facing surfaces:
    • Result summary (line 7229) now emits an additional "Recommendation: ..." line directly below the existing "Disposition: S-PFS X — Functioning ..." line. Conditional on dispositionRecommendation being non-null, so failed/aborted sessions (which don't get a disposition at all) cleanly omit both lines.
    • CSV export gets a new dispositionRecommendation column appended after dispositionSpfs. Column header at line 4086, cell value at line 4150. Existing CSV column order before the new column is unchanged so spreadsheets joining on column index stay compatible.
  - Surfaces NOT changed (deliberately, per "keep both" choice):
    • The CPA technical breakdown block at line 7432 still displays only the cognitive-state Disposition line. This block is research-context; recommendations would clutter it.
    • The verification payload's provisionalScores.disposition field (line 5641) still contains the cognitive-state label. This field is part of the receipt-verification hash and changing its meaning would invalidate every previously-uploaded session's receipt. Explicit decision: verification keeps using dispositionLabel; the recommendation rides along the rest of the saved-row payload but isn't part of the verification hash.
    • The session-list rendering loop at line 12476 still builds its line from dispositionLabel. Same research-context rationale.
    • The Cognitive Performance Reference Table (Figure 7 in the manuscript and the in-app explainer) is unchanged. It describes the cognitive state, which is what it should describe; adding action recommendations there would conflate two different concepts.
  - Verification (27 assertions across 4 scenario groups):
    1. Per-band recommendations: each of disp 1..7 returns the correct verbatim string.
    2. Band groupings match Layne's spec: 7-6-5 share one string (Set size 1), 2-1 share one string (Set size 1), 4 distinct from 5, 3 distinct from 4, 3 distinct from 2.
    3. Defensive cases: null/undefined/empty/0/8/-1/NaN all return null; "7" string-coerces to FIT; 3.4 rounds to STRONG; 3.6 rounds to CAUTION.
    4. Universality: no domain-specific words ("Buc-ee", "kiosk", "motel", "store", "highway", "cogspeed", "trial", "phase") leaked into any recommendation string. Recommendations stay portable across deployments.
    5. Polarity sanity: disp 7 (best in re-oriented convention) starts with "Fit"; disp 1 (worst in re-oriented) starts with "Stop". A future contributor who flipped the polarity by mistake would fail this check.
  - Version stamps bumped V699rev174 → V699rev175 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine / manifest.json name. APP_VERSION remains V699. No other content changed.
  ...
  - Six new admin defaults added to DEFAULTS (lines 110ff) and to the field-list (entries #90-95). All six default to the rev169 hardcoded values, so any existing device whose saved settings predate rev170 will inherit identical behavior on first load via the existing DEFAULTS-merge in loadSettings():
    ...
    2. baselineMinSessionsToEstablish (#91, default 5) — qualifying-session count needed before the FIRST Baseline is established. Decoupled from window size.
    3. baselineMinSpfs (#92, default 5) — minimum qualifying S-PFS value.
    4. baselineMaxSpfs (#93, default 7) — maximum qualifying S-PFS value.
    5. baselineAllowedModes (#94, default "mode1,mode2") — comma-separated testMode list. Empty/null falls back to the default. Trim-tolerant.
    6. baselineExcludeFailed (#95, default 1) — when 1 (default), failed sessions are skipped; when 0, they're allowed to qualify. Any non-zero value is treated as "exclude" for safety.
  - Six accessor helpers added (getBaselineWindowSize / getBaselineMinSessionsToEstablish / getBaselineMinSpfs / getBaselineMaxSpfs / getBaselineAllowedModes / getBaselineExcludeFailed) each with hardcoded fallbacks to the rev169 default if the setting is missing, malformed, or out of range. This means a corrupted-settings device still produces the rev169 baseline behavior rather than failing.
  - isBaselineQualifyingSession() rewritten to read from the new accessors instead of using literal "mode1"/"mode2" / 5 / 6 / 7 / always-exclude-failed checks. Output is byte-identical at default settings (verified by sandbox run); changes only appear when admin values are altered.
  - computePersonalBaseline() generalized:
    • The hardcoded literal `4` (5th-qualifying-session establishment threshold) is now `getBaselineMinSessionsToEstablish() - 1`.
    • The hardcoded slice width `5` (window size) is now `getBaselineWindowSize()`. The window start is `max(0, i - windowSize + 1)`, which gracefully handles the case where minToEstablish < windowSize (early Baselines simply average fewer-than-windowSize qualifying sessions; later Baselines use the full window once enough qualifying sessions exist).
    • Each Baseline entry now records `windowSize` and `minSessionsToEstablish` so the rule-set in force at establishment time travels with the historical record.
    • Return object adds `windowSize`, `minSessionsToEstablish`, and `lastWindow` (window-size-aware) fields. `lastFive` is retained as a legacy alias so the two existing consumers (openPersonalBaselinePage display fallback and buildBaselineUploadContext.usedNowCount) continue to work.
  - Display strings made admin-driven so user-facing text always reflects the configured rule:
    • Personal Baseline graph legend — "Qualifying session (in current rolling N)" and "Baseline established (rolling N-session avg)" now interpolate getBaselineWindowSize().
    • Personal Baseline overlay meta panel — describes the establishment rule with the configured min-to-establish and window size, the configured S-PFS range, the configured allowed modes (rendered through getFullModeLabel for human-readable mode names like "Mode 2 CogSpeed Sustained"), and the failed-session policy.
    • "Established Baselines (rolling N-session averages)" table header reads from getBaselineWindowSize(); per-row "ms (rolling N-session average)" reads from each Baseline's stamped windowSize so older entries computed under a different window display correctly even after admin changes.
    • Results-summary BASELINE definition tooltip (line 6991) — full text driven from the six accessors so the explanation matches the runtime rule. Falls back to "Mode 1 / Mode 2" wording at defaults; reads "Mode 1" or "Mode 2" alone when only one is enabled.
    • Tutorial Step 6 Personal Baseline explainer reads from the same accessors.
  - Defensive notes:
    • baselineWindowSize=1 produces a "rolling average of 1" (i.e., every qualifying session establishes a Baseline equal to its own MBS) — supported as an explicit setting in case research scenarios need it; not the recommended default.
    • baselineMinSessionsToEstablish < baselineWindowSize: early Baselines use a smaller-than-windowSize window (avg of however many qualifying sessions exist at establishment time). This is the deliberate decoupling Layne requested — research designs may want quick first-establishment but a long stable window for ongoing tracking.
    • baselineMinSpfs > baselineMaxSpfs: produces zero qualifying sessions (vacuous range). No crash; just an empty baseline. Admins can use this to temporarily disable Baseline establishment without wiping history.
    • baselineAllowedModes can be set to a single mode (e.g. "mode2") to produce mode-pure baselines; can include modes not currently exposed in the test-mode picker (e.g. "mode1,mode2,mode3") if a research protocol later adds Mode 3 sessions.
  - Verification (23 assertions across 7 scenarios) confirms: defaults reproduce rev169 behavior exactly; window=3+minToEst=3 produces 6 baselines from 8 sessions with correct 3-session averages; window=10+minToEst=5 establishes B1 at 5th session with avg of all 5, eventually covering all 8 sessions in the average by B4; S-PFS 6..7 (stricter) drops S-PFS=5 sessions; allowedModes="mode1" produces zero baselines on a Mode-2-only history but full baselines on a Mode-1-mixed history; excludeFailed=0 keeps failed sessions, =1 drops them; window > minToEst correctly produces fall-short windows for early baselines.
  - Version stamps bumped V699rev169 → V699rev170 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine / manifest.json name. APP_VERSION remains V699. No CPA, audio, scheduler-notice, copyright/license, or technical-overview content changed.


  - Specification (Layne, V699 rev 169): once 5 qualifying sessions exist for a registered subject, that 5-session MBS average becomes a permanently retained Baseline. Every subsequent qualifying session creates a new rolling 5-session-average Baseline, and every Baseline ever established is recorded forever. The Personal Baseline graph displays every qualifying session AND every established Baseline. The Results display always shows the latest established Baseline. Qualifying criteria unchanged: Mode 1 or Mode 2 (Layne confirmed both modes remain in scope), MBS ≤ 1900 ms (admin setting personalBaselineMaxMbs), S-PFS 5/6/7, non-Guest, non-failed.
  - computePersonalBaseline now returns a `baselineHistory` array — the canonical sequence of every Baseline derivable from state.history. Iteration starts at the 5th qualifying session (orderIndex=4) and emits one Baseline per qualifying session thereafter. Each entry carries baselineNumber (1-based), value, establishedAt (the triggering session's time), triggeringQualifyingOrderIndex (graph anchor), triggeringSessionIndex (raw history index), and the full window indices. The function's `averageMbs` field is now sourced from the most recent baselineHistory entry when established, so getPersonalBaselineSummaryText and renderSpeedometerBaseline (used by Results, Speedometer, Outcome, and email/clipboard exports) automatically pick up the latest Baseline value with no further changes.
  - Two-tier persistence (per Layne's "Both" choice):
    1. Per-session snapshot — at finish-save time, a `baselineSnapshot` object is written onto every saved result with version, established flag, latestBaselineNumber, latestBaselineValueMs, latestBaselineEstablishedAt, baselineCountSoFar, thisSessionEstablishedNewBaseline, and thisSessionWasQualifying. This captures the Baseline state as observed AT the moment that session was saved and travels with the row in localStorage.
    2. Per-subject permanent cache — new localStorage key `${STORAGE_PREFIX}_baseline_history` storing { [subjectId]: [BaselineEntry…] }. New helpers loadPersistedBaselineHistoryCache / savePersistedBaselineHistoryCache / clearPersistedBaselineHistoryCache mirror the existing history-persistence pattern, and clearPersistedHistory now also clears this cache. New state field state.baselineHistoryCache holds the in-memory copy.
  - Reconciliation: new helper reconcileBaselineHistoryForSubject(subjectId) merges the persistent cache with the freshly derived sequence on every read. Behavior: for each derived entry, if the cache already contains a record at the same baselineNumber + establishedAt + value, the cached record wins verbatim (preserves any future fields added to historical records). If the trigger matches but the value drifted (e.g. a window session was re-coded later), the derived value wins. Cached Baselines whose baselineNumber exceeds the derivable count are kept and tagged `orphaned:true` — this is what makes Baselines "permanently retained" even if a triggering session is later deleted; the historical Baseline record itself survives. Reconciliation runs whenever the subject's Baseline data is read for display; if the merged result differs from the cached result, the merged result is written back. New helpers getBaselineHistoryForSubject and getLatestBaselineForSubject expose the merged sequence to callers.
  - Save-path integration: in the FINISH_SAVE block, after pushing the new result onto state.history but before serializing, reconcileBaselineHistoryForSubject is called for the session's subject to (a) update the persistent cache with any newly established Baseline and (b) generate the per-session snapshot. This guarantees the cache is up-to-date and the snapshot reflects whether THIS session itself triggered a new Baseline (thisSessionEstablishedNewBaseline). Wrapped in try/catch so a snapshot failure cannot block the test save itself.
  - Personal Baseline graph (buildPersonalBaselineSvg) — overlay redesign:
    • Now accepts a third argument `baselineHistory` and plots two superimposed traces on the same y-axis. Sessions: the existing cyan polyline + green/yellow dots (green for sessions used in the current rolling 5, yellow for older ones), unchanged. Baselines: a pink (#ff7fb0) dashed polyline connecting every established Baseline's value, plotted at the x-position of its triggering qualifying-session order index, with a 10×10 rotated diamond marker at each established Baseline. Each diamond is labeled "B1", "B2"… directly above so users can correlate the graph with the Baselines table beneath.
    • A horizontal green dashed reference line + "Latest Baseline {N} ms" annotation marks the most recent Baseline value across the chart width. The legend (new, four items: green circle = "Qualifying session (in current rolling 5)", yellow circle = "Qualifying session (older)", pink diamond = "Baseline established (rolling 5-session avg)", green dashed line = "Latest Baseline") is laid out in two rows of two below the x-axis caption so it always fits at the 860-wide viewBox.
    • Y-scale extension: max/min computation now folds in baseline values too, so when the rolling Baseline drifts above or below the session range, the chart auto-expands to keep both traces visible.
    • Title updated to "Personal Baseline — Qualifying Sessions & Established Baselines". X-axis caption simplified ("Qualifying session order (oldest to newest)") since the rolling-5 callout is now redundant with the legend.
  - Personal Baseline overlay (openPersonalBaselinePage):
    • Status text now reads "Latest Baseline: {N} ms (Baseline #{n})" when established, replacing the prior "Baseline: {N} ms" so users see both the current value AND which Baseline it is in the historical sequence.
    • Meta panel adds a "Baselines established: {n}" line and replaces the old single-paragraph rolling-baseline note with a clearer description of the establishment rule + permanent-retention guarantee.
    • Table extended: after the existing per-session rows + "Current rolling baseline average" footer, a new "Established Baselines" section appears with one row per Baseline showing baselineNumber, establishedAt timestamp, and value. Orphaned (historical) Baselines whose triggering session was later deleted are flagged "(historical — triggering session removed)" in italic muted text.
  - Functional sanity check (16 assertions across 7 scenarios) confirms: <5 qualifying sessions produces no Baseline; the 5th establishes Baseline #1 with the correct rolling average; the 6th establishes Baseline #2 with the correctly shifted window; non-qualifying sessions (S-PFS 4, MBS>1900, failed, mode4) are skipped without breaking the chain; mixed Mode 1+2 sessions both contribute; subjects do not pollute each other's baselines; Guest subject IDs produce no Baseline.
  - Version stamps bumped V699rev168 → V699rev169 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine. APP_VERSION remains V699. No CPA, admin, audio, scheduler-notice, copyright/license, or technical-overview content changed.

- Rev168 session-label standardization across all selectors and exports:
  - Audit found at least five different session-selector dropdowns in the app, each using a different label format. The same session could read four different ways depending on which page was open: "Sess 4 · M2 Sustained · 4/25/2026" on the Summary page; "Session 4 · Mode 2 CogSpeed Sustained · ranger@example.com · 4/25/2026, 3:42:18 PM" on the Trial Log; etc. Same-day sessions could not be distinguished because the date-only format omitted the time. Failed sessions were indistinguishable from successful ones in every label. Mode 2 sessions that never reached the sustained phase (no CPA) looked identical to Mode 2 sessions that did.
  - New canonical helper formatSessionListLabel(result, idx, opts) composes a single uniform label format: "Sess N · {compactMode}{(no CPA)} · [subjectHint] · {date} {time}{⚠ FAILED}". Trigger marker "(no CPA)" is appended for testMode==="mode2" sessions where mode2Triggered===false so users can tell at a glance which Mode 2 sessions actually produced a CPA score. Time is always included so two same-day sessions are visually distinct. Failed sessions (any session where isTestSuccess returns false) are flagged with "⚠ FAILED". subjectHint is opt-in (showSubject:true) for the Trial Log and Rate-RT Chart selectors which can span subjects on multi-user devices; trimmed to 14 chars so long emails do not crowd the option text.
  - Five session selectors patched to use the canonical helper:
    1. syncSummarySessionSelect (Summary page)
    2. syncSpeedometerSessionSelect (Speedometer overlay)
    3. syncTrialLogSessionSelect (Trial Log overlay)
    4. buildRateRtOverlay (Rate-RT chart selector + info bar)
    5. syncResponseGraphSessionSelect (Response Graph)
  - Two additional surfaces updated for label consistency:
    • Rate-RT chart highlighted-session canvas overlay now reads "Highlighted: Sess N · {compactMode}" (was "Highlighted: Session N") so the in-canvas highlight is unambiguous when multiple sessions of different modes are overlaid.
    • formatLastPerfTimeText (perf-time clipboard/email export) now prepends the canonical session label to each row so exported text is self-describing — a user pasting it into a spreadsheet can immediately tell Mode 1 from Mode 2 from a Mode 2 with no CPA from a failed session.
  - Two helpers renamed for clarity:
    • formatModeTag → getFullModeLabel. The old name was misleading: it returned the FULL mode label ("Mode 2 CogSpeed Sustained"), not a short tag. Use formatCompactResultModeLabel for the short tag form. All 13 call sites updated.
    • getLatestCompletedMode2Label → getLatestCompletedMode2Timestamp. The old name was misleading: it returned a localized timestamp string, not a label. The single call site (scheduler status panel) was updated.
  - Personal-baseline session table at line 4701 was left unchanged — it is a structured HTML table with idx, time, mode, MBS, S-PFS, and "used in current baseline" each in their own &lt;td&gt; cell, which is a different display affordance from the dropdown labels and reads cleanly column-by-column.
  - Version stamps bumped V699rev167 → V699rev168 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine / manifest.json name. APP_VERSION remains V699. No CPA, admin, audio, scheduler-notice, or technical-overview content changed.
- Rev167 consolidated build — Rev 159 architectural baseline + Rev 160-166 keepers:
  - Background: an external Rev 166 audited build was found to have reverted Rev 151+153+154+155+157+158+159 work wholesale, including the entire Rev 151 CPA v2 architecture (accuracy composite + OLS drift + CV with weights 9/6/6 sum 21 against the cap of 20), the Rev 151 safe-migration block, the Rev 153 results-summary cleanup (Drift ratio line removed), the Rev 154 iOS WebAudio unlock primer, the Rev 155 Survival silent-switch notice, the Rev 157 mobile scheduler background-limitation notice, the Rev 158 Mode 4 two-line yellow box, and both Rev 159 Tech Overview deliverables. In addition, Mode 3 and Mode 4 speedometer prep code had been deleted entirely so those modes' yellow boxes were rendering empty. Rev 167 takes the Rev 159 build as the architectural baseline and ports forward only the Rev 166 changes that represent genuine new work.
  - Audit verdict on Rev 166: only two of the four claimed Rev 166 changes were actually new code. The "dual-needle Mode 2 secondary needle" rendering and the "wrap.style.gridTemplateColumns boxes-count guard" were already present in Rev 159; the Rev 166 V699rev166-tagged comments around them were credit-rewrites on existing code, not changes. The "state.speedometerMode2Metric removed" credit is similarly misattributed — that removal happened in Rev 141.
  - Rev 166 keeper #1: new tutorial page 1. Replaced a 17-item generic device-care preparation list with a tighter 9-item, emotionally-aware list. Opens with "CogSpeed is not an IQ test. Just do your best." Reframes "go fast" guidance with "There are no Right or Wrong scores." Explicitly addresses the "I can't keep up" reaction with "Skip a frame if needed" plus "CogSpeed is always faster than you are!" Promotes the safety message to a final bold MOST IMPORTANT! item with sub-bullets ending in "STOP AND REST! → regardless of your score." Genuine UX improvement; real keeper.
  - Rev 166 keeper #2: manifest stale-literal fix. The "name" field in manifest.json had been stuck at "CogSpeed® V699rev141" through every revision. Bumped to "CogSpeed® V699rev167". (Rev 167 will continue to bump this with each future build.)
  - Everything else: Rev 167 retains the full Rev 159 architectural baseline. Rev 151 CPA v2 architecture, Rev 151 admin retune (weights 9/6/6 sum 21 vs cap 20), Rev 151 getInterpolatedBucketValue piecewise-linear bucket interpolation, Rev 151 mode2-cpa-norm-v2 model stamp, Rev 151 6-column CSV expansion, Rev 151 safe-migration IIFE, Rev 153 results-summary cleanup, Rev 154 iOS WebAudio unlock + sync-resume patch, Rev 155 iOS Survival silent-switch notice, Rev 157 mobile Scheduler background-limitation notice (iOS + Android), Rev 158 Mode 4 yellow-box two-line layout (RT row + Rate row), Rev 159 Tech Overview rev-independence rewrite, and Rev 159 "Why Mode 2 is the primary mode" relocation to §4 Mode architecture — all confirmed intact at audit time.
  - APP_REV_STAMP bumped to V699rev167, sw.js RELEASE bumped to 699rev167, index.html app.js cache-bust query bumped to v=699rev167, tab title / versionBadge / statusLine literals bumped, manifest.json name bumped. APP_VERSION remains V699.
- Rev159 Technical Overview document revision:
  - Made the document version/rev-independent. Removed all references to specific app revisions (V699rev151, V699rev150, "Rev 151 build", "Under the earlier Rev 150 design", "current build", "earlier builds") so the document describes CogSpeed's current design and rationale without tying the prose to any particular release stamp. The "Mode 2 CPA normative v2" model version is retained because it refers to the scoring-model identifier carried in research-upload receipts, not to an app release.
  - Moved "Why Mode 2 is the primary mode" from §12 (Use cases, limitations, deployment cautions, and references) to the top of §4 (Mode architecture), placed immediately after the four-row mode table. That location is where the reader first encounters the mode lineup and the question "why this mode over the others" naturally arises. The §6 CPA opening paragraph retains its own brief recapitulation of the three-part design because it frames the CPA formula that immediately follows; the two framings serve different readers and together reinforce the central design argument.
  - Retitled one subsection: §6 "Retired features (retained as diagnostics)" became "Non-CPA sustained-phase diagnostics" so the name describes what the items are (supplementary measures available in results and CSV) rather than how they fit into a past revision's history. Admin-settings-item-number citations replaced with a generic phrase since the numbers are exposed in-app and tied to the build.
  - Wording cleanups: "In the current build..." phrasings removed from §3 (Scheduler paragraph), §8 (Personal baseline), and §10 (Research-consent placeholder). The research-consent sentence now states the present state directly instead of gating it on "this build".
  - No other content changed. Document structure, TOC numbering, and sponsor/contact block remain intact.
  - Unrelated: version stamps bumped V699rev158 to V699rev159 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine. No CPA, admin, CSV, audio, or scheduler-notice code changed.
- Rev158 Mode 4 speedometer yellow-box two-line layout:
  - Fixes the Mode 4 speedometer value string that previously crammed both the measured average RT and the fixed machine-pacing Rate onto a single line, separated only by a bullet: "450.0 ms • Rate 550.0 ms". The two numbers were visually ambiguous (which was the response time, which was the pacing?) and competed for horizontal space, triggering the auto-fit logic and shrinking the font to near-floor on narrow canvases.
  - New layout renders two distinct lines inside the same yellow box: "RT 450.0 ms" on the top row and "Rate 550.0 ms" on the bottom row, each with its own label so the two values are no longer ambiguous. Two-line font sized at R*0.058 with a floor of R*0.042, each row auto-fit independently against the box inner width.
  - drawSpeedometer gains an optional opts.tipValueSecondary; when present, the yellow box renders two rows at 30% and 72% of box height; when null/empty it falls back to the original single-line centered layout unchanged. Mode 2 and Mode 3 speedometers (which need only one value) are unaffected.
  - Mode 4 rendering block at the speedometer entry point now builds speedoOpts.tipValueSecondary = "Rate {pacedRate} ms" when both avg RT and pacing rate are defined, falls back cleanly to single-line when only one is available. Matches the existing Mode 2 guard pattern for speedoOpts initialization.
  - Unrelated: version stamps bumped V699rev157 to V699rev158 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine. No CPA, admin, CSV, Technical Methods, or audio code changed.
- Rev157 Scheduler background-limitation notice generalized to Android + iOS:
  - Replaces the Rev 156 iOS-only "iPhone / iPad note" on the MODE 2 SCHEDULER panel with a unified "Mobile browser note" that displays on both iPhone/iPad and Android devices. Text: "Mobile browser note: scheduled reminders are most reliable when this browser tab is open and on-screen. If the tab is backgrounded for more than a minute, or the browser is closed, iPhone and Android operating systems may throttle or suspend the reminder timer, in which case the alert will be late or will not play at all. For a reminder that works when CogSpeed is closed, also set an alarm in the Clock app for the same time."
  - Rationale: the Rev 156 notice implied the limitation was iOS-specific. Android Chrome in fact throttles setTimeout in backgrounded tabs to roughly 1 call per minute and may pause it entirely after extended background time, and delivers zero events when the browser process is closed. Without server-side Web Push (not yet implemented; tracked as a future work item), neither platform can reliably fire scheduled reminders while the app is closed. The rewritten notice sets honest expectations on both platforms.
  - Implementation: element id renamed iosSchedulerBackgroundNotice → mobileSchedulerBackgroundNotice. CSS visibility gate changed html.is-ios → html.is-mobile. A new "is-mobile" class is added to the html element alongside the existing "is-ios" class by the extended platform-detection IIFE (installPlatformNoticesRev157, which replaces installIosSilentSwitchNotice). Detection is userAgent /iPad|iPhone|iPod/ plus iPadOS-13+-spoofs-Mac fallback plus /Android/ substring test. The Survival silent-switch notice from Rev 155 remains iOS-only since hardware-silent-switch gating is a genuine iOS-only quirk.
  - Future-work note: this remains a documentation / UX fix, not a technical fix. The actual reliable-reminder solution on both iOS 16.4+ and Android is server-side Web Push via the service worker. That requires a backend push server, VAPID key pair, and a push-event handler in sw.js. Scoped as a separate project.
  - Unrelated: version stamps bumped V699rev156 to V699rev157 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine. No CPA, admin, CSV, Technical Methods, or audio code changed.
- Rev156 iOS Scheduler background-limitation notice:
  - Adds an amber-bordered informational banner at the top of the MODE 2 SCHEDULER panel on the Start Page, visible only on iPhone/iPad. Text: "iPhone / iPad note: scheduled reminders play only while this browser tab is open and on-screen. iOS does not allow a web page to play sound or show notifications when its tab is closed or the browser is in the background. For a reminder that works when CogSpeed is closed, also set an alarm in the Clock app for the same time."
  - Rationale: iOS suspends JavaScript execution in non-visible tabs within roughly 60 seconds of backgrounding, and closed tabs run no code at all. Web Push (the Android workaround that fires OS-level notifications from a closed browser) is only supported on iOS for Home-Screen PWAs running iOS 16.4 or later, which CogSpeed as currently deployed is not. Users who rely on a Scheduler reminder without knowing this limitation miss their scheduled tests; the notice points them at the Clock-app workaround.
  - Visibility gate is pure CSS: html.is-ios #iosSchedulerBackgroundNotice { display:block !important }. The is-ios class is already set at script load by the Rev 155 installIosSilentSwitchNotice IIFE, so no new JS was needed. Invisible on Android, desktop, or any non-iOS device. Always visible on iOS regardless of Scheduler ON/OFF state, because the limitation applies to every schedule.
  - Unrelated: version stamps bumped V699rev155 to V699rev156 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine. No CPA, admin, CSV, Technical Methods, or audio code changed.
  - Future-work note: the long-term fix for closed-tab scheduled reminders on iOS is to deploy CogSpeed as a Home-Screen PWA and implement iOS-16.4+ Web Push, which requires a backend push server. That is a separate platform-architecture change beyond the scope of this revision.
- Rev155 iOS-only Survival silent-switch notice:
  - Adds a conditionally-visible informational banner on the Start Page that appears ONLY when (a) the device is iPhone or iPad, AND (b) the Test Type selector is set to "Mode 2 CogSpeed Survival Challenge". Text: "iPhone / iPad note: for the Survival Challenge sound effects, the hardware silent switch on the side of your device must be OFF (no orange showing). iOS mutes all web-page audio when this switch is on, even when media and phone sounds are working normally."
  - Rationale: iOS Safari and Chrome-on-iOS (which is forced by Apple to use WebKit) route WebAudio output through an audio session category that obeys the hardware silent switch. A device with the silent switch ON produces no Survival SFX even when the ringer is unmuted and other apps play audio normally. This is a platform-level gate we cannot work around in code; the notice saves the support round-trip.
  - iOS detection uses the standard userAgent regex (iPad | iPhone | iPod) plus a fallback for iPadOS 13+ which spoofs MacIntel in the UA and is detected via navigator.maxTouchPoints > 1 on a Mac. "is-ios" class is added to the html element so CSS can gate iOS-specific affordances without JS re-branching at render time. A change listener on the test-type selector toggles "show-for-survival" on the notice element; visibility is controlled purely by the CSS intersection of both classes.
  - Zero impact on Android, desktop, or iOS users who pick any test type other than Survival. Notice is invisible in every other combination.
  - Unrelated: version stamps bumped V699rev154 to V699rev155 in APP_REV_STAMP, sw.js RELEASE, cache-bust query / tab title / versionBadge / statusLine. No CPA, admin, CSV, or Technical Methods content changed.
- Rev154 iOS WebAudio unlock fix for Survival Challenge SFX:
  - Root cause: on iOS Safari, even with the hardware silent switch OFF, a newly created AudioContext can be in state "running" but still produce no speaker output until the audio session has been primed via audio actually scheduled from a user gesture. ctx.resume() alone was not sufficient. Prior to Rev 154 this path had no iOS unlock step, which would produce totally silent Survival Challenge sound effects on iPhone and iPad despite the device ringer being on.
  - Fix #1: one-shot iOS unlock primer (installIosWebAudioUnlock) installed at script load. On the first touchstart / pointerdown / click / keydown anywhere in the document, it creates (or reuses) the shared Survival AudioContext, resumes it, and plays an inaudible (0.00001 gain) 25ms oscillator burst into ctx.destination. This is the standard iOS WebAudio unlock pattern. Listeners use the capture phase so upstream stopPropagation can't shadow them and { once: true } so the primer only fires once per page load.
  - Fix #2: synchronous-resume-first pattern in playSurvivalCorrectSound. The prior code (Promise.resolve(ctx.resume()).then(()=>setTimeout(emit,0))) deferred emit through a microtask and a macrotask, which can lose user-gesture context on iOS Safari and cause the first correct-tap SFX to be silent. Rev 154 attempts ctx.resume() synchronously and emits immediately if ctx.state flips to "running" in-frame; falls back to the promise path otherwise. No behavior change for non-iOS engines.
  - No CPA logic, admin defaults, safe-migration, CSV columns, or technical-overview content changed. Version stamps bumped V699rev153 → V699rev154 in APP_REV_STAMP, sw.js RELEASE, index.html cache-bust query / tab title / versionBadge / statusLine.
  - Deployment note: iOS Safari is stubborn about service-worker updates. If Rev 154 still seems silent on first load after deploy, close the tab and reopen (or Share → Close All Tabs in Safari) to force the new service worker to take over the cache.
- Rev153 Mode 2 results-summary cleanup pass on Rev152:
  - Removed the three bracketed "[Legacy correct/wrong/missed-response factor]" lines from the Mode 2 CPA section. The bracketed correct-response value was an exact duplicate of the Sustained accuracy-composite factor printed directly above it, and the bracketed wrong/missed lines were advertising retired internals the user does not need.
  - Removed the three stand-alone "(retired V699rev151)" lines for Recovery / calibration RT factor, Lapse-rate factor, and Block-efficiency factor. These were CPA weighting factors that Rev 151 already stopped populating; printing a row of dashes with a retirement note forever is clutter. The raw diagnostic values derived from the same inputs (Recovery÷calib RT ratio, Sustained-phase lapse rate, Block formation efficiency) are retained because they stand on their own as useful measurements.
  - Removed the legacy "Drift ratio: X%" line (median-of-halves, non-negative clamped). It contradicted the OLS drift values printed two lines above it — for any run where the subject spent up in the second half, the OLS line showed a meaningful negative percentage while the Drift ratio line reported 0.0%. Early/Late median sustained RT lines retained since those are useful RT descriptors on their own.
  - The Mode 2 CPA block is now 16 lines instead of 23, with no retired-internals chatter and no contradictory drift readouts.
  - Fixed three stale "V699rev151" literals in index.html (tab title, versionBadge, statusLine) that Rev 152 missed. syncReleaseUI() rewrites them at runtime, but bumping them at source prevents the first-paint flash on slow mobile loads.
  - APP_REV_STAMP bumped to V699rev153, sw.js RELEASE bumped to 699rev153, index.html app.js cache-bust query bumped to v=699rev153, tab title / versionBadge / statusLine literals bumped. APP_VERSION remains V699. CPA scoring logic, admin defaults, safe-migration block, CSV columns, and Technical Methods document are all unchanged from Rev 151/152.
- Rev152 surgical fix: buildScoringSnapshot now includes CPA v2 accuracy-composite expected/tolerance/weight keys in the exported admin-field snapshot for research/AWS verification.
- Rev151 Mode 2 CPA architecture overhaul (fixes structural issues identified in the Rev 150 CPA audit):
  - (a) Accuracy-composite collapse. The three previously collinear accuracy features (correct / wrong / miss rates, which sum to ~1 and therefore triple-counted the accuracy signal) are consolidated into a single composite metric: accComposite = correctRate − wrongRate − 0.5·missRate. Weight 9.0, tolerance 0.15. The three raw rates continue to be computed and reported as diagnostics and are retained in the CSV column layout.
  - (b) Linear expected-profile interpolation. Bucket lookups for all Mode 2 CPA expected profiles now use piecewise-linear interpolation between bucket centers (CPI 10/30/50/70/90) with flat extrapolation below 10 and above 90. Adjacent CPI values now produce smoothly adjacent expected profiles, eliminating the step discontinuities at CPI = 20 / 40 / 60 / 80 that caused hysteresis near the boundaries. New helper getInterpolatedBucketValue. getBucketValue retained unchanged for any other callers.
  - (c) OLS-slope drift estimator. The drift feature now uses an ordinary-least-squares slope of correct sustained RT vs 1-indexed trial position, rescaled to the same "percent change from first to last trial" interpretation. Uses every correct trial (not a median-of-halves split). Signed — a negative slope (within-phase speed-up) earns a positive residual instead of being floored to zero. Falls back to the legacy median-of-halves estimator at fewer than 3 correct trials. Legacy earlyMedian / lateMedian / driftRatio diagnostics are still reported for transparency.
  - (d) Weight / cap architecture rebuild. Old weights 3.0 + 2.5 + 3.5 + 1.5 + 1.5 = 12 left the mode2NormMaxDelta cap of 20 vestigial. New weights: accuracy 9.0, OLS drift 6.0, CV 6.0, sum 21.0. The cap is held at 20 points so it CAN engage at the extreme of underperformance (all three features at −1 saturation) while no single feature alone saturates the cap under typical operation.
  - Admin defaults: #75 mode2NormWeightCorrect retired (3.0 → 0), #76 mode2NormWeightWrong retired (2.5 → 0), #77 mode2NormWeightMiss retired (3.5 → 0), #78 mode2NormWeightDrift upgraded (1.5 → 6.0, now drives OLS-slope residual), #79 mode2NormWeightCv upgraded (1.5 → 6.0). New settings: #87 mode2NormExpectedAccuracyComposite (bucket spec, defaults derived from old expected rates), #88 mode2NormToleranceAccuracyComposite (0.15), #89 mode2NormWeightAccuracy (9.0). Retired and deprecated entries retained in Admin and in DEFAULTS for storage continuity; labels marked RETIRED / Deprecated with Rev151 provenance.
  - Safe migration block migrateRev151CpaArchitectureSafely only replaces stored values that still equal the prior defaults, preserving genuine user edits. One-time per-revision per-device stamp stored in localStorage.
  - CPA model version bumped from 'mode2-cpa-norm-v1' to 'mode2-cpa-norm-v2' in research-upload payloads so server-side verifiers can dispatch to the correct scoring implementation.
  - Results summary Mode 2 relabeled: primary "Sustained accuracy-composite factor" line plus bracketed legacy lines for the retired correct / wrong / miss factors; new diagnostic lines print observed/expected accuracy composite, OLS drift slope (ms/trial), and OLS full-phase drift % so the score trace is inspectable. Results-Metric Explanations section rewritten to describe the three-feature V699rev151 architecture with formulas and rationale.
  - CSV export: six new columns added after cpaEfficiencyWeighting — cpaAccuracyWeighting, cpaAccuracyResidual, cpaObservedAccuracyComposite, cpaExpectedAccuracyComposite, cpaObservedDriftSlopeMsPerTrial, cpaObservedDriftPctOls. Existing CPA columns retained in place so existing research pipelines continue to parse.
  - Technical Methods document (technical-overview-v92.html) §5 and §6 rewritten with the Rev 151 CPA architecture, formulas, feature definitions with rationale, cap design discussion, interpolation explanation, retired-feature note, and normative-status disclosure.
  - APP_REV_STAMP bumped to V699rev151, sw.js RELEASE bumped to 699rev151, index.html app.js cache-bust query bumped to v=699rev151, tab title / versionBadge / statusLine literals bumped. APP_VERSION remains V699.
- Rev150 three-bug fix pass on Rev149:
  - Speedometer label auto-shrink in drawSpeedometer so Mode 3 "Average Self-paced RT" and Mode 4 "Average Machine-Paced RT" (plus the Mode 4 value line with rate) fit inside the yellow MBS box without overflowing the speedometer arc. Base font sizes preserved; ctx.measureText only shrinks when needed, floored at a legible minimum.
  - Performance over Date and Time zoom-controls layout fix: Range dropdown now occupies its own row and the three Zoom Out / Reset / Zoom In buttons share an even three-column row beneath it, restoring the intended balanced layout (Rev147 introduced a nested-grid issue that stacked all four controls into the left third of a three-column row).
  - Stale "V699rev141" literals in index.html (browser tab title, versionBadge, statusLine) bumped to V699rev150 so first-paint before syncReleaseUI() runs no longer shows a stale version string.
  - APP_REV_STAMP bumped to V699rev150, sw.js RELEASE bumped to 699rev150, index.html app.js cache-bust query bumped to v=699rev150. APP_VERSION remains V699.
...
## Rev 87
- Shortened Results/Speedometer session selector labels for phone screens: "Session" → "Sess", removed subject/email, shortened stamp to date only, and compacted mode names to M1 Adapted / M2 Sustained / M3 Self-paced / M4 Machine-paced.

## Rev 86 — Personal Baseline always shows all qualifying sessions
- Rebuilt from Rev 85 with the Personal Baseline overlay behavior explicitly retained: whenever Personal Baseline is shown, the graph and table show **all qualifying sessions** available as of that result, not only the rolling five.
- The rolling baseline value itself still uses the **most recent 5 qualifying sessions**.
...
- Renamed the in-app technical page to `technical-overview-v76.html` and updated About, Results, Profile, and Admin links so the cleaned navigation page is the active route.
- Bumped the cache-busting script tag to `app.js?v=699rev76`, bumped `sw.js` RELEASE to `699rev76`, and advanced `APP_REV_STAMP` to `V699rev76`. `APP_VERSION` remains `V699` because this is a UI-cleanup/navigation revision only.
...
- Renamed the in-app technical page to `technical-overview-v76.html` and updated About, Results, Profile, and Admin links to force a fresh safe HTML route instead of any stale cached document.
- Kept the persistent `Back to CogSpeed` / `Return to App` navigation on the technical overview page.

## Rev 74
- Removed the in-app `Open PDF version` buttons from `technical-overview-v76.html` so phone/PWA users cannot be stranded outside the CogSpeed app shell by launching the PDF viewer.
- Kept `technical-overview-v76.html` as the only in-app technical-document route, with explicit return buttons back to `index.html`. The bundled PDF remains in the package for archival/reference use only and is no longer exposed through the app UI.
- Bumped the cache-busting script tag to `app.js?v=699rev76`, bumped `sw.js` RELEASE to `699rev76`, and advanced `APP_REV_STAMP` to `V699rev76`. `APP_VERSION` remains `V699` because this is a safe-navigation revision only.

## Rev 73
- Made `technical-overview-v76.html` the primary destination for all in-app technical-document entry points. The About page secondary button, the Profile research placeholder button, and the Admin overlay button now all route to the in-app HTML overview instead of opening the PDF directly.
- Added persistent explicit return navigation on `technical-overview-v76.html`: a fixed top `← Back to CogSpeed` button plus bottom return actions so phone/PWA users do not get stranded outside the app shell.
- Kept the PDF as a secondary option only on `technical-overview-v76.html` via `Open PDF version`, opened in a separate target so the HTML page remains the safe primary route.
...
- Bumped the cache-busting script tag to `app.js?v=699rev71`, bumped `sw.js` RELEASE to `699rev71`, and advanced `APP_REV_STAMP` to `V699rev71`. `APP_VERSION` remains `V699` because this is a behavioral/UI revision only.
- Bug fix (duplicate event listener): removed a redundant `document.addEventListener("visibilitychange", ...)` registration that was present both in the Scheduler init block (line ~8472) and in the end-of-file global listener block (line ~9842). Both copies invoked the same `maybeFinishBackgroundTest()` handler, so every return-to-foreground was firing the handler twice. Guards inside `maybeFinishBackgroundTest()` made this behaviorally idempotent for now, but any future addition of logic that runs before the `backgroundTestPending` flag flip would have produced a double-confirm. Consolidated to a single registration at the end of the file alongside the other global listeners.
- Bug fix (scheduler sound test false negatives on iOS): `onSchedulerTestSound()` previously ANDed `const heard = ok && confirm(...)`, which silently marked the sound test FAIL whenever the `HTMLAudioElement` fired `onabort` or `onerror`. iOS Safari is known to fire `onabort` *after* successful cached playback under memory pressure or when the confirm dialog steals focus mid-playback, so users who actually heard the sound were being recorded as FAIL without ever being asked. The user is now always prompted with the "Did you hear the alert sound?" confirm; the playback event result is kept only for the status bar (an internal note of "playback event reported failure" is surfaced when the user says no AND the event system also reported failure, to distinguish "user couldn't hear it" from "audio pipeline broke").
- Scheduler sound overlap protection: `playSchedulerSound()` now tracks the currently-playing `Audio` on `schedulerState.activeAudio` and `pause()`s any prior playback before starting a new one. Rapid taps on the Test Sound button were previously spawning overlapping Audio objects that all raced to resolve the Promise and played concurrently. The new single-playback discipline keeps the test deterministic.
- Survival sound limiter ducking: `playSurvivalCorrectSound()` now tracks `state._lastSurvivalSoundMs` and applies a linear duck factor to `masterGain` when successive correct-tap sounds arrive within 120 ms of each other (factor is 1.0 at ≥120 ms gap, 0.55 at 0 ms gap, linear between). Previously, consecutive fast correct taps (sub-400 ms RT in Survival mode) could stack 15–20 oscillator/buffer sources each with tails up to 1.8 s, slamming the limiter into heavy compression and making subsequent hits sound muffled. Ducking preserves every hit as audibly distinct while keeping the summed signal in the limiter's linear range. No per-component gain values were changed; all Rev 63 tuning is preserved.
- Mobile memory hygiene: the Survival `AudioContext` stored on `state._survivalAudioCtx` is now `close()`d when the tab becomes hidden (via the existing `visibilitychange` handler) and recreated lazily on the next call to `playSurvivalCorrectSound()`. Frees ~5–15 MB on backgrounded PWA installs and prevents Chrome's autoplay state machine from pinning the context in a running-but-silent state. Guarded so a `close()` failure on older Safari does not break visibility handling.
- `about.html` additions: added an "Audio playback notes" section describing iOS/iPadOS ambient audio session behavior, how calls/Siri/other app audio can interrupt or duck CogSpeed sounds, and operational guidance (close other audio apps before a session; set volume before starting; keep ringer switch unmuted). Makes the sound-interruption behavior documented field-observable rather than surprising.
- Explicit non-change: a wrong-answer audio cue was considered and deliberately not added. Introducing error-feedback sound mid-test would alter the stimulus and confound RT-distribution and error-rate comparisons against prior-rev data stored under the same `APP_VERSION = "V699"` storage prefix. The only sound-producing cue during a test remains the Survival correct-tap family cue, unchanged from Rev 63.
- Bumped the cache-busting script tag to `app.js?v=699rev70`, bumped `sw.js` RELEASE to `699rev70`, and advanced `APP_REV_STAMP` to `V699rev70`. `APP_VERSION` is unchanged at `V699` because this revision is behavioral-only — no storage-schema changes, no wipe of existing registered-user profiles.
...
- Added new supplemental methods document `technical-overview-v76.html` plus bundled `technical-overview.pdf` titled **CogSpeed Technical Overview: Test Logic, Scoring, Baseline Construction, and Use Considerations**.
...
- Rev62 no-repeat probe field-name fix — the Rev 60b "next probe must never equal the immediately previous probe" rule was silently a no-op in Memory and Survival modes because of a field-name mismatch between the caller and the generators. The caller at `openTrial()` builds `lastProbe={family:state.current.probeFamily, count:state.current.probeCount}`, but `makeMemoryTrial()` and `makeIconChallengeTrial()` both checked `lastProbe.num===probeNum`. Since `lastProbe.num` was always `undefined`, the check `undefined===<number>` always evaluated false and the `continue` was never hit — meaning Memory and Survival probes could repeat back-to-back despite the changelog claim of a hard no-repeat rule. Standard (non-challenge) mode was always correct because it reads `.family` and `.count`, which matches what the caller builds. Rev 62 changes both Memory and Survival generators to read `lastProbe.count` (the field the caller actually passes), so the hard no-repeat rule now correctly applies in all three probe generators across all modes. No other code was touched for this fix — the caller, field names in the returned trial object, and the 500-attempt retry loop are all unchanged.
- Rev62 bug fix — "next test should always be Mode 2" was not actually happening for returning users. Root cause: `openProfileOverlay()` read `existing.symbolSet` from the saved profile record (which `saveAndContinueProfile()` had been writing since earlier builds) and re-applied it via `profileSelectSymbolSet(existingSymbolSet)` on every profile-open. That call silently overrode the Rev 50/58/61 post-test reset to Mode 2 Sustained by putting the stale challenge set back into `settings`. The Rev 61 `normalizeLiveTestTypeForProfileOpen()` did set mode2/standard first, but was immediately overwritten by the existingSymbolSet re-application a few lines later. Rev 62 removes the `existingSymbolSetRaw`/`existingSymbolSet` derivation and both `profileSelectSymbolSet()` calls from `openProfileOverlay()`; the unified Test Type dropdown is now driven solely by the normalized live state via `getUnifiedProfileTestType()`. This is the actual fix that matches what Rev 50/57/58/59/61 were trying to do.
- Rev62 profile-record cleanup — `saveAndContinueProfile()` no longer writes `symbolSet` into the saved profile record. Test type is now a live-only setting (per the Rev 50 "Profile is the only place that changes the active test mode" policy that was partially implemented but leaked via the profile record). Removing the `symbolSet` field from the saved profile eliminates the source of the stale value that was being resurrected on every profile-open.
- Rev62 one-time per-revision migration — added an `APP_REV_STAMP` check right after `repairChallengeAdminDefaults()`. On first load of each new rev on a given device, this migration (a) forces `settings.testMode="mode2"` and `settings.symbolSet="standard"` (the admin default), and (b) strips the legacy `symbolSet` field out of any saved profile record left over from earlier revs, then writes the current rev stamp. Subsequent loads see the stamp match and skip the migration. This fulfills the "Admin default #2 should start every new revision with Mode 2" requirement for returning users whose localStorage settings blob still carried a stale test-mode selection from a prior rev.
- Rev62 survival sounds — upgraded four of the six family cues to better match the visual identity of the impacts:
  - Tank/cannon (icons 3,4): slower, lower-pitched muzzle crack (spark + powder crack body + mid ring) sequenced before a longer main boom at +50 ms, plus a 550 ms resonant barrel-ring tail sweeping 58→34 Hz. Reads as "distant cannon firing" rather than "rifle shot."
  - Jets (icons 1,2): redesigned as aircraft-exploding. Approach whoosh → detuned metallic shred tones (sawtooth 1800→620 Hz and 1350→480 Hz) → bandpass-like torn-metal crackle → main fuel-fireball boom → 500 ms tumbling-debris low rumble on the opposite pan.
  - ICBM/missile battery (icon 8): split out of the shared `rocket` family into its own `missile` family with `getSurvivalSoundFamily(8)`. Profile is a pure detonation (no launch whoosh — the launch is over, this is the target impact): 100 ms pre-compression sub ramp 45→95 Hz → heavier main boom at +100 ms → secondary sub-octave thump → dual-side debris noise → 700 ms long-low pressure-wave tail 62→30 Hz.
  - Helicopter/RPG (icons 11,12): switched from whoosh-forward to explosion-forward. Short 80 ms RPG approach whoosh → main blast boom at +60 ms → bright mid-band secondary crack (rotor/fuselage fragment) → low secondary thump → 360 ms debris rumble tail on the opposite pan.
  - Icon 7 (rocket), ship (5,6), and space (9,10) cues are unchanged from Rev 27.

## V699 — 2026-04-19 (Rev 61 and earlier)
...
- Rev47 bug fix: saved-profile auto-restore can no longer get stuck on a clear-and-continue policy case. Prior revs' `restoreSubjectFromProfile()` only read the passive `getSingleUserDevicePolicyMessage(email)` and bailed out whenever it was non-empty, which correctly blocked the "mixed local histories" case but also incorrectly blocked the two cases the device policy says should CLEAR-AND-CONTINUE — Guest-only persisted history plus a signed-in saved profile, and legacy different-user persisted history plus a signed-in saved profile. Manual sign-in via the Subject-ID Next button already routed through `enforceSingleUserDevicePolicy()`, which handles both cases by confirming and clearing. Auto-restore now delegates to the same enforcer for those resolvable cases, so the two entry points behave consistently. Rev 47 preserves the existing mixed-case UX (hint-only, no alert dialog on back-button navigation) by keeping the passive-message path for `status: "mixed"`. The enforcer is only called for `status: "guest"` or `status: "user"` with a candidate mismatch. If the enforcer returns false (operator canceled the confirm, or mixed) the restore falls through to the hint path exactly as before. If the enforcer approved a different-user clear the saved profile was wiped as part of that clear, so the restore then detects `loadProfile()` returns null and bails to the empty-profile branch rather than reinstating a stale in-memory profile.
- Rev46 bug fix: Mode 4 baseline factor fallback constant. The Rev 45 change bumped `DEFAULTS.mode4BaselineFactor` from 1.3 to 1.1 and updated the admin field label, but the runtime fallback literal in `finishCalibration()` was left at the old value: `const factor=Number(settings.mode4BaselineFactor)||1.3`. In the edge case where `settings.mode4BaselineFactor` coerces to 0, NaN, empty string, or any other falsy value, the fallback silently applied 1.3 — inconsistent with the new 1.1 default visible everywhere else. Rev 46 replaces the hardcoded literal with `DEFAULTS.mode4BaselineFactor` so the fallback can never drift out of sync with the canonical default again. Any future change to the default updates the fallback automatically.
- Rev46 dead-code removal: `hasInMemoryGuestOnlyRows()` (declared in Rev 44) and `clearGuestOnlyHistoryIfPresent()` (declared in Rev 42) were both written as helpers during the Guest-policy evolution but were never called anywhere in the codebase. Rev 44 settled on checking `hasPersistedGuestHistory` directly inside `getSingleUserDeviceOwnerState()`, making `hasInMemoryGuestOnlyRows` an unused leftover; Rev 43's `loadPersistedHistory()` migration and `savePersistedHistory()` defensive filter fully cover the "strip Guest rows from disk" responsibility, making `clearGuestOnlyHistoryIfPresent` redundant. Both helpers removed to keep the Guest-policy surface area minimal and prevent future confusion about which function is authoritative.
- Rev45 admin default update: Mode 4 MP baseline factor from calibration average (Admin field 46) is now 1.1 instead of 1.3.
- Rev44 bug fix: device-owner policy now ignores transient in-memory Guest session rows, so Guest current-session views no longer make a signed-in device look mixed during the same app run.
- Rev43 Guest non-persistence enforcement — critical fix for the Rev 42 policy gap. Rev 42 declared that "Guest sessions are not stored locally" in the device-policy state machine, and wrote the `shouldPersistSessionForLocalHistory()` helper, but the helper was never wired into the save path in `finish()`. Every Guest session was still being written to `${STORAGE_PREFIX}_history` just like any other session. Rev 43 gates the `savePersistedHistory(state.history)` call in `finish()` (line 2033) on `shouldPersistSessionForLocalHistory(result)`, so Guest results now stay in `state.history` for the current session's speedometer and summary view but never touch localStorage. On page refresh the Guest row is gone from disk, so it cannot contaminate a signed-in user's CSV export, Performance-over-time chart, Summary session dropdown, or Speedometer session dropdown.
- Rev43 defensive write filter — added `stripGuestRowsForDisk()` and applied it inside `savePersistedHistory()` so any future caller (admin tools, backup/restore, future import paths) that tries to write a list containing Guest rows has those rows silently dropped from the on-disk payload. The function returns the in-memory list unchanged so current-session views are unaffected. Belt-and-suspenders safety net against repeat instances of the Rev 42 gap.
- Rev43 legacy-guest migration on load — `loadPersistedHistory()` now strips Guest rows from any pre-Rev-43 installs where Guest sessions leaked onto disk under the Rev 42 policy gap, and writes the cleaned list back. Returning users who ran Guest sessions on a signed-in device under Rev 42 will see those rows removed from their local history on first launch of Rev 43.
- Rev43 backup-restore guard — the restore path now also strips Guest rows from the backup payload before writing to disk, so a backup file created under Rev 42 cannot re-introduce Guest sessions on a restore. Restore-side sibling of the Rev 43 load-time migration.
- Rev42 device policy update: Guest is now allowed on a device that already contains a signed-in user's local history, but Guest sessions are not stored locally.
- Rev42 device policy update: when a signed-in user starts on a device that contains Guest-only local history, the app clears the Guest-only sessions first and then continues.
- Rev42 device policy update: when a different signed-in user starts on a device that already contains another signed-in user's local history, the app warns that local data will be cleared and requires confirmation before continuing.
- Rev41 single-user device policy: one device can keep only one local user history at a time.
- Rev41 single-user device policy: if a different email or Guest is entered on a device that already contains another user's local history, the app blocks entry and tells the operator to clear local data first.
